...
def question(request, pk):
    ...
    return render(request, "question.html", {"ques": ques,"score":tester.total_score,"time":newtime,"sen":sen,"pk":pk,"junior":tester.junior})
...
def mysubmission(request, pk):
    ...
    sub_counter=[]
    k=0
    if(Player.objects.get(user=request.user).junior==False):
        k=2
    for i in range(1,7):
        ...
        if(custom):
            ...
            if correct_tcs == tc_count:
                return render(
                    request,
                    "base.html",
                    {"result": "AC", "testcase": cases,'score':scr,'pk':pk,"time":newtime}
                )
            else:
                return render(
                    request,
...
def logout_view(request):
    ...
    if(tester.junior==False):
        k=3
    if(len(Submission.objects.filter(p_id=Player.objects.get(user=request.user), q_id=k))):
        attempted+=1
        if(tester.ques1==100):
            solved+=1
    if(len(Submission.objects.filter(p_id=Player.objects.get(user=request.user), q_id=k+1))):
        attempted+=1
        if(tester.ques2==100):
            solved+=1
    if(len(Submission.objects.filter(p_id=Player.objects.get(user=request.user), q_id=k+2))):
        attempted+=1
        if(tester.ques3==100):
            solved+=1
    if(len(Submission.objects.filter(p_id=Player.objects.get(user=request.user), q_id=k+3))):
        attempted+=1
        if(tester.ques4==100):
            solved+=1
    if(len(Submission.objects.filter(p_id=Player.objects.get(user=request.user), q_id=k+4))):
        attempted+=1
        if(tester.ques5==100):
            solved+=1
    if(len(Submission.objects.filter(p_id=Player.objects.get(user=request.user), q_id=k+5))):
        attempted+=1
        if(tester.ques6==100):
            solved+=1
    logout(request)
    return render(request, "result.html", {"first":first,"second":second,"third":third,"leaders": data,"rank":rank,"tester":tester,"solved":solved,"attempted":attempted})